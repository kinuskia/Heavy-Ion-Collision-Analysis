...
for p in range(1, len(percentiles)):
	...
	source_result = 'output/one_point_random_' + str(percentiles[p-1]) + '-' + str(percentiles[p])  +'.txt'
	profile = np.loadtxt(source_result)
	source_error = 'output/one_point_random_' + str(percentiles[p-1]) + '-' + str(percentiles[p]) + '_error' +'.txt'
	...
	filename = "plots/one_point_random_" + str(percentiles[p-1]) + "-" + str(percentiles[p]) + ".pdf"
