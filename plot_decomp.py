...
plt.figure(1)
profile = np.loadtxt('output/decomposition_modulus0.txt')
...
plt.figure(3)
profile = np.loadtxt('output/decomposition_phase0.txt')
plt.imshow(profile, interpolation=None, cmap=plt.cm.Blues)
plt.xlabel("$l-1$")
plt.ylabel("$m$")
plt.title("$\\phi_{m,l}$")
plt.colorbar()
plt.savefig("plots/FB_coeffs_phase.pdf", format='pdf', bbox_inches = "tight")