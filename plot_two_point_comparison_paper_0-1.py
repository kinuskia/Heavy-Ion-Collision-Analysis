...
figs, axs = plt.subplots(len(modes), 4, figsize=(6.0,8.0),sharex="col", sharey="row", constrained_layout=True)
...
for mode in modes:
	...
	im1 = axs[mode][0].imshow(profile[0:(lMax),0:(lMax)], interpolation=None, cmap=cmap_Trento, vmin = -maximal_T, vmax = maximal_T, extent = (-0.5+1, len(profile[0,0:(lMax)])-0.5+1, len(profile[0:(lMax),0])-0.5+1, -0.5+1))
	#centrality_class =  str(p) + '-' + str(p+1) + '%'
	if (mode == 0):
		axs[mode][0].set_title("TRENTo")
	if (mode == modes[-1]):
		axs[mode][0].set_xlabel("$l_2$")
		figs.colorbar(im1, ax = axs[mode][0:2], shrink = 1, pad=0.0, aspect = 40, location = "bottom")
	axs[mode][0].set_ylabel("$m={0}$\n$l_1$".format(mode))
	axs[mode][0].set_xticks(np.arange(lMax)+1)
	axs[mode][0].set_yticks(np.arange(lMax)+1)
...
for mode in modes:
	...
	axs[mode][1].imshow(profile[0:(lMax),0:(lMax)], interpolation=None, cmap=cmap_LargeNc, vmin = -maximal_N, vmax = maximal_N, extent = (-0.5+1, len(profile[0,0:(lMax)])-0.5+1, len(profile[0:(lMax),0])-0.5+1, -0.5+1))
	#centrality_class =  str(p) + '-' + str(p+1) + '%'
	if (mode == 0):
		axs[mode][1].set_title("Large $N_c$")
	if (mode == modes[-1]):
		axs[mode][1].set_xlabel("$l_2$")
	axs[mode][1].set_xticks(np.arange(lMax)+1)
	axs[mode][1].set_yticks(np.arange(lMax)+1)
	#ax.set_ylabel("$m={0}$\n$l_1$".format(mode))
...
for mode in modes:
	...
	im2 = axs[mode][2].imshow(profile[0:(lMax),0:(lMax)], interpolation=None, cmap=cmap_IPSM, vmin = -maximal_I, vmax = maximal_I, extent = (-0.5+1, len(profile[0,0:(lMax)])-0.5+1, len(profile[0:(lMax),0])-0.5+1, -0.5+1))
	#centrality_class =  str(p) + '-' + str(p+1) + '%'
	if (mode == 0):
		axs[mode][2].set_title("IPSM")
	if (mode == modes[-1]):
		axs[mode][2].set_xlabel("$l_2$")
		figs.colorbar(im2, ax = axs[mode][2:4], location = "bottom", shrink = 1, pad=0.0, aspect = 40)
	axs[mode][2].set_xticks(np.arange(lMax)+1)
	axs[mode][2].set_yticks(np.arange(lMax)+1)
	#ax.set_ylabel("$m={0}$\n$l_1$".format(mode))
...
for mode in modes:
	...
	axs[mode][3].imshow(profile[0:(lMax),0:(lMax)], interpolation=None, cmap=cmap_magma, vmin = -maximal_C, vmax = maximal_C, extent = (-0.5+1, len(profile[0,0:(lMax)])-0.5+1, len(profile[0:(lMax),0])-0.5+1, -0.5+1))
	#centrality_class =  str(p) + '-' + str(p+1) + '%'
	if (mode == 0):
		axs[mode][3].set_title("magma")
	if (mode == modes[-1]):
		axs[mode][3].set_xlabel("$l_2$")
	axs[mode][3].set_xticks(np.arange(lMax)+1)
	axs[mode][3].set_yticks(np.arange(lMax)+1)
