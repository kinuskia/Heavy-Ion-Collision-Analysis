...
plt.plot(x, y10, label = "$l=1$")
plt.plot(x, y20, label = "$l=2$")
plt.plot(x, y30, label = "$l=3$")
plt.plot(x, np.zeros(200), linestyle = "dotted", color="gray")
#plt.plot(x, y40, label = "$J_0\\left(z_4^{(0)} \\rho\\right)$")
plt.xlabel("$\\rho$")
plt.ylabel("$J_0\\left(z_l^{(0)} \\rho\\right)$")
plt.legend(loc = "best")
plt.title("$m=0$")
...
plt.figure(2)
plt.rcParams.update({'font.size': 15})
plt.plot(x, y11, label = "$l=1$")
plt.plot(x, y21, label = "$l=2$")
plt.plot(x, y31, label = "$l=3$")
plt.plot(x, np.zeros(200), linestyle = "dotted", color="gray")
#plt.plot(x, y40, label = "$J_0\\left(z_4^{(0)} \\rho\\right)$")
plt.xlabel("$\\rho$")
plt.ylabel("$J_1\\left(z_l^{(1)} \\rho\\right)$")
plt.title("$m=1$")
plt.legend(loc = "best")
plt.savefig("plots/Bessel1.pdf", format = "pdf", bbox_inches = "tight")
plt.close(2)
