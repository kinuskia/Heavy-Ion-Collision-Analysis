...
// overloads for STL vectors
template<typename number_type>
void insert(number_type number, size_type counter, std::vector<std::vector<number_type>> & data)
{
...
template<typename number_type>
void read_data(std::string filename, std::vector<std::vector<number_type>> & data, size_type skip_rows = 0)
{
	std::ifstream infile(filename);
	std::string line;
